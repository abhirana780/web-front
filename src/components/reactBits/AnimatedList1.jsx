    ...
    <div className={`relative w-[600px] ${className}`}>
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div className={`p-4 bg-[#111] rounded-lg my-4! ${selectedIndex === index ? 'bg-[#222]' : ''} ${itemClassName}`}>
              <p className={`m-0 ${selectedIndex === index ? 'text-red-500' : 'text-white'}`}>{item.sno} - {item.name} - {item.jobrole}</p>
            </div>
          </AnimatedItem>
        ))}
      </div>
